...
| 问题 2 | 固定电价（附件 1）+ 负载与**光伏实际功率**（附件 2），逐日 0:00 计划（**计划用附件 1 的光伏预测**），缺口按 5 倍价紧急购电 | 全年（2025-02-01—12-31，334 天）**18 092 018.06 元**（计划 12 352 438.44 + 紧急 5 739 579.62），紧急购电量 **1 373 687.3 kWh**（334/334 天出现） |
...
| 问题 4-2 | 波动电价（附件 4）下重算问题 2 | **19 281 482.14 元**（计划 12 774 413.66 + 紧急 6 507 068.48） |
| 问题 4-3 | 波动电价下重算问题 3 | **17 702 310.51 元** |

**核心建模结论**：若"计划与实际同源"（理想情形：已知实际光伏），储能的唯一作用是时段套利（峰谷价比 3.2 远大于往返效率门槛 1.235），
全天购电量恒等于"负载 − 光伏 + 储能循环损耗"；**一旦计划基于预报、结算基于实际光伏，紧急购电就成为主要成本项**
（问题 2 的紧急购电费占总费用 31.7%），且**预报越准越省**——问题 3 用附件 3 的每日预报比问题 2 的典型日预报省 1 121 099 元（−6.6%）。
因此模型改进的重点是**如何在不确定预报下做出可执行的购电计划**。

> **v3 修正（重要）**：v1/v2 曾把附件 2 的"光伏发电**实际**功率"直接当作日前计划输入，导致问题 2／4-2 的紧急购电恒为 0。
> v3 依据附录 2 的列名（附件 1 = 光伏**预测**功率、附件 2 = 光伏**实际**功率）与"附件 1 恰为全年平均剖面"的数据核查，
> 恢复了"计划用预测、结算用实际"的信息结构。问题 1／3／4-3 与五项改进不受影响。详见 v3 文档 §4 与 §16.1。
...
- **储能寿命（EFC 口径）**：最优磨损价 0.104 元/kWh（LCOS 折算），完全信息情形全年"能量+寿命"费再降 40 546 元，
...
│   │   ├── result1.xlsx  result2.xlsx  result3.xlsx  result4-2.xlsx  result4-3.xlsx  ← result2/result4-2 为 v3 修正版
...
$PY src/write_results.py    # 输出 result1/3/4-3.xlsx（v1 基线）
$PY src/make_figures.py && $PY src/dump_tables.py

# ---- v3：修正问题 2 / 4-2 的信息结构（计划用预测、结算用实际）----
$PY src/v3_p2.py            # 重算问题 2 与 4-2 → out/p2_v3.npz, p42_v3.npz
$PY src/v3_write_results.py # 重写 result2.xlsx / result4-2.xlsx + 表 1/2/3 格式
$PY src/v3_figures.py       # v3 图表
...
| 要求—实现对照（v4） | 26 条题面要求逐条对照实现与位置（v4 文档 §17.2）；13 条常见陷阱逐条给出处理结论（§17.3） |
| 信息结构专项复核（v3） | 问题 2 的计划按**预报**建立的平衡裕量恒为 0（约束收紧）；改用**实际**光伏检验则为 −247 kWh 量级（必然出现缺口），确认"计划用预测、结算用实际"已正确落地 |
