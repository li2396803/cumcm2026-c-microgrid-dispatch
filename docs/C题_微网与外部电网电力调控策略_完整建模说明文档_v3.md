...
**版本说明**：v1 → v2 落地了 5 项模型改进（§11–§15）并新增"假设逐条复核"（§16）；**v3 在 v2 基础上修正了问题 2／4-2 的信息结构错误**——v1/v2 误把附件 2 的**实际**光伏当作日前计划输入，导致紧急购电恒为 0；现改为"计划用附件 1 的**光伏预测**、结算用附件 2 的**实际光伏**"，问题 2／4-2 的结果与结果文件随之修正（见 §16.1）。
